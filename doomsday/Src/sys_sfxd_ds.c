...
IDirectSoundBuffer8 *DS_DSoundCreateBuffer8(DSBUFFERDESC *desc)
{
	...
	if(FAILED(hr = IDirectSound_CreateSoundBuffer(dsound, desc, 
		&buf, NULL))) return NULL;
	
	// Obtain the DirectSoundBuffer8 interface.
	if(FAILED(hr = IDirectSoundBuffer_QueryInterface(buf,
		&IID_IDirectSoundBuffer8, &buf8))) buf8 = NULL;
...
IDirectSound3DBuffer8 *DS_DSoundGet3D(IDirectSoundBuffer8 *buf8)
{
	...
	if(FAILED(hr = IDirectSoundBuffer_QueryInterface(buf8,
		&IID_IDirectSound3DBuffer8, &buf3d)))
	{
...
//===========================================================================
// DS_EAXHasSupport
//	Returns true if getting and setting the property is supported.
//===========================================================================
boolean DS_EAXHasSupport(int property)
{
	...
	if(!eax_listener) return false;
	
	has_support = SUCCEEDED(hr = IKsPropertySet_QuerySupport(eax_listener, 
		&DSPROPSETID_EAX_ListenerProperties, property, &support))
		&& (support & EAXSUP) == EAXSUP;

	if(verbose) Con_Message("DS_EAXHasSupport: Property %i => %s\n",
		property, has_support? "Yes" : "No");
...
//	Returns true if EAX is available.
//===========================================================================
int DS_EAXInit(void)
{
	...
	if(ArgExists("-noeax")) return false;
	...
	if(FAILED(hr = IDirectSound3DBuffer_QueryInterface(dummy3d,
		&IID_IKsPropertySet, &eax_listener))) return false;	
...
int DS_DSoundInit(void)
{
	...
	if(dsound) return true; // Already initialized?

	// First try to create the DirectSound object with EAX support.
	hr = DSERR_GENERIC;
	if(!ArgExists("-noeax"))
	{
		if(SUCCEEDED(hr = CoCreateInstance(&CLSID_EAXDirectSound8, NULL, 
			CLSCTX_INPROC_SERVER, &IID_IDirectSound8, &dsound)))
		{
			if(verbose) Con_Message("DS_DSoundInit: DS8/EAX instance created.\n");
		}
	}
	if(FAILED(hr))
	{
		// Try plain old DS, then.
		if(FAILED(hr = CoCreateInstance(&CLSID_DirectSound8, NULL, 
			CLSCTX_INPROC_SERVER, &IID_IDirectSound8, &dsound)))
		{
	...
	if(FAILED(hr = IDirectSound_SetCooperativeLevel(dsound,
		hWndMain, DSSCL_PRIORITY)))
	{
	...
	{
		// Get the listener.
		if(FAILED(hr = IDirectSoundBuffer_QueryInterface(primary,
			&IID_IDirectSound3DListener, &ds_listener)))
		{
...
void DS_DSoundLoad(sfxbuffer_t *buf, struct sfxsample_s *sample)
{
	...
	if(FAILED(hr = IDirectSoundBuffer_Lock(DSBuf(buf), 
		0, 0, &data, &locked_bytes, 0, 0, DSBLOCK_ENTIREBUFFER)))
		return; // Couldn't lock!

	// Write as much data as we can.
	memcpy(data, sample->data, 
		wrote_bytes = MIN_OF(locked_bytes, sample->size));
...
void DS_DSoundPlay(sfxbuffer_t *buf)
{
	...
	if(buf->flags & SFXBF_RELOAD) DS_DSoundLoad(buf, buf->sample);
...
void DS_DSoundRefresh(sfxbuffer_t *buf)
{
	...
	for(i = 0; i < 2 && data[i]; i++)
	{
		...
		if(dose)
		{
			...
			memcpy((byte*) data[i], 
				(byte*) buf->sample->data + buf->written, dose);
			buf->written += dose;
			buf->cursor += dose;
		}
		if(dose < bytes[i])
		{
			...
			if(!(buf->flags & SFXBF_REPEAT))
			{
				...
				memset((byte*) data[i] + dose, buf->bytes == 1? 128 : 0, fill);
				buf->cursor += fill;
			}
		}
	...
	IDirectSoundBuffer_Unlock(DSBuf(buf), 
		data[0], bytes[0], 
		data[1], bytes[1]);
...
int DS_DSoundLogPan(float pan)
{
	...
	if(pan == 0) return 0;
	if(pan > 0) return -100 * 20 * log10(1 - pan);
...
void DS_DSoundSet(sfxbuffer_t *buf, int property, float value)
{
	...
	{
	case SFXBP_VOLUME:
		IDirectSoundBuffer_SetVolume(DSBuf(buf), 
			value <= 0? (-1-value) * 10000	// Use logarithmic attenuation.
			: DS_DSoundLinLog(value));// Linear volume.
		break;
	...
	case SFXBP_PAN:
		IDirectSoundBuffer_SetPan(DSBuf(buf), 
			DS_DSoundLogPan(value));
		break;

	case SFXBP_MIN_DISTANCE:
		if(!DSBuf3(buf)) return;
		IDirectSound3DBuffer_SetMinDistance(DSBuf3(buf), value,
			DS3D_DEFERRED);
		break;

	case SFXBP_MAX_DISTANCE:
		if(!DSBuf3(buf)) return;
		IDirectSound3DBuffer_SetMaxDistance(DSBuf3(buf), value,
			DS3D_DEFERRED);
		break;
	...
	case SFXBP_RELATIVE_MODE:
		...
		IDirectSound3DBuffer_SetMode(DSBuf3(buf), 
			value? DS3DMODE_HEADRELATIVE : DS3DMODE_NORMAL,
			DS3D_DEFERRED);
		break;
	}
}
...
void DS_DSoundListenerOrientation(float yaw, float pitch)
{
	...
	IDirectSound3DListener_SetOrientation(ds_listener,
		front[VX], front[VY], front[VZ],
		up[VX],	up[VY],	up[VZ], 
		DS3D_DEFERRED);
}
...
//===========================================================================
// DS_EAXHasFailed
//	Returns true if the specified property has failed.
//===========================================================================
boolean DS_EAXHasFailed(DWORD prop)
{
	...
	for(i = 0; i < MAX_FAILED_PROPS; i++)
		if(failed_props[i] == prop) return true;
	return false;
}

//===========================================================================
// DS_EAXReportError
//	Returns true if an EAX error should be reported. hr must be set.
//===========================================================================
boolean DS_EAXReportError(DWORD prop)
{
	if(ignore_eax_errors) return false;
	if(hr != DSERR_UNSUPPORTED) return true;
	if(DS_EAXHasFailed(prop)) return false; // Don't report again.
...
{
	if(FAILED(hr = IKsPropertySet_Set(eax_listener, 
...
void DS_EAXMuldw(DWORD prop, float mul)
{
	...
	if(FAILED(hr = IKsPropertySet_Get(eax_listener, 
		&DSPROPSETID_EAX_ListenerProperties,
		prop, NULL, 0, 
		&value, sizeof(value), &retBytes)))
	{
		if(DS_EAXReportError(prop))
...
void DS_EAXCommitDeferred(void)
{
	if(!eax_listener) return;
	IKsPropertySet_Set(eax_listener, 
		&DSPROPSETID_EAX_ListenerProperties,
...
void DS_DSoundListenerEnvironment(float *rev)
{
	...
	DS_EAXSetdw(DSPROPERTY_EAXLISTENER_ROOM, 
		DS_DSoundLinLog(rev[SRD_VOLUME]));

	// Reverb decay.
	val = (rev[SRD_DECAY] - .5)*1.5 + 1;
	DS_EAXMulf(DSPROPERTY_EAXLISTENER_DECAYTIME, val, 
...
void DS_DSoundListenerv(int property, float *values)
{
	...
	case SFXLP_POSITION:
		if(!ds_listener) return;
		IDirectSound3DListener_SetPosition(ds_listener, 
			values[VX], values[VZ], values[VY], DS3D_DEFERRED);
		break;

	case SFXLP_VELOCITY:
		if(!ds_listener) return;
		IDirectSound3DListener_SetVelocity(ds_listener, 
			values[VX], values[VZ], values[VY], DS3D_DEFERRED);
		break;

	case SFXLP_ORIENTATION:
		if(!ds_listener) return;
		DS_DSoundListenerOrientation(values[VX]/180*PI, values[VY]/180*PI);
		break;
