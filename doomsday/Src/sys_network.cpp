...
//**
//** $Log$
//** Revision 1.5.2.1  2004/05/16 10:01:35  skyjake
...
void N_ReturnBuffer(void *handle)
{
	if(netServerMode)
	{
		gServer->ReturnBuffer((DPNHANDLE)handle, 0);
	}
	else
	{
		gClient->ReturnBuffer((DPNHANDLE)handle, 0);
	}
}
...
void N_ReceiveMessage(PDPNMSG_RECEIVE received)
{
	...
	msg->handle = (void*) received->hBufferHandle;
...
boolean N_GetNodeName(nodeid_t id, char *name)
{
	...
	if((hr = gServer->GetClientInfo(id, info, &size, 0)) ==
	   DPNERR_BUFFERTOOSMALL)
	{
		// Allocate enough memory and get the data.
		info = (DPN_PLAYER_INFO*) calloc(size, 1);
		info->dwSize = sizeof(*info);
		gServer->GetClientInfo(id, info, &size, 0);
		strncpy(name, (const char*) info->pvData, 255);
		free(info);
		return true;
	}
	else
...
boolean N_LookForHosts(void)
{
	...
	if(gEnumHandle) 
	{
		...
		if(gHostCount)
		{
			...
			for(hostnode_t *it = gHostRoot; it; it = it->next)
				Net_PrintServerInfo(it->index, &it->info);
		}
		return true;
	}
