                              ...
                              float shadowDark)
{
    ...
    rseg->texScale[1] = size;
    rseg->texOffset[1] = calcTexCoordY(top, *fFloor, *fCeil, rseg->texScale[1]);
    ...
    {   // At least one corner faces outwards
        rseg->texture = LST_RADIO_OO;
        rseg->texScale[0] = radioConfig->spans[TOP].length;
        rseg->texOffset[0] =
        ...
        {   // Both edges are open
            ...
            if(radioConfig->topCorners[0].proximity && radioConfig->topCorners[1].proximity)
            {
                if(-radioConfig->topCorners[0].pOffset >= 0 && -radioConfig->topCorners[1].pOffset < 0)
                {
                    ...
                    if(size > -radioConfig->topCorners[0].pOffset)
                    {
                        ...
                        {
                            rseg->texScale[1] = -radioConfig->topCorners[0].pOffset;
                            rseg->texOffset[1] =
                                calcTexCoordY(top, *fFloor, *fCeil, rseg->texScale[1]);
                        }
                    }
                }
                else if(-radioConfig->topCorners[0].pOffset < 0 && -radioConfig->topCorners[1].pOffset >= 0)
                {
                    ...
                    if(size > -radioConfig->topCorners[1].pOffset)
                    {
                        ...
                        {
                            rseg->texScale[1] = -radioConfig->topCorners[1].pOffset;
                            rseg->texOffset[1] =
            ...
            else
            {
                if(-radioConfig->topCorners[0].pOffset < -MINDIFF)
                {
                    // Must flip horizontally!
                    rseg->texture = LST_RADIO_OE;
                    rseg->texScale[0] = -radioConfig->spans[BOTTOM].length;
                    rseg->texOffset[0] =
            ...
            if(radioConfig->bottomCorners[0].proximity && radioConfig->bottomCorners[1].proximity)
            {
                if(radioConfig->bottomCorners[0].pOffset >= 0 && radioConfig->bottomCorners[1].pOffset < 0)
                {
                    ...
                    if(size > radioConfig->bottomCorners[0].pOffset)
                    {
                        ...
                        {
                            rseg->texScale[1] = -radioConfig->bottomCorners[0].pOffset;
                            rseg->texOffset[1] =
                                calcTexCoordY(top, *fFloor, *fCeil,
                                              rseg->texScale[1]);
                        }
                ...
                else if(radioConfig->bottomCorners[0].pOffset < 0 && radioConfig->bottomCorners[1].pOffset >= 0)
                {
                    ...
                    if(size > radioConfig->bottomCorners[1].pOffset)
                    {
                        ...
                        {
                            rseg->texScale[1] = -radioConfig->bottomCorners[1].pOffset;
                            rseg->texOffset[1] =
                                calcTexCoordY(top, *fFloor, *fCeil,
                                              rseg->texScale[1]);
                        }
...
void Rend_RadioSetupSideShadow(rendseg_shadow_t* rseg, float size, float bottom,
                               ...
                               float shadowDark)
{
    float mul = sideCn[rightSide? 1 : 0].corner;

    rseg->shadowDark = shadowDark * (mul * mul * mul);
    rseg->texOffset[1] = *fCeil - top;
    rseg->texScale[1] = *fCeil - *fFloor;

    if(rightSide)
    {   // Right shadow.
        rseg->texOffset[0] = -(lineLength) + xOffset;
        // Make sure the shadow isn't too big
        if(size > lineLength)
        {
            if(sideCn[0].corner <= MIN_OPEN)
                rseg->texScale[0] = -lineLength;
            else
                rseg->texScale[0] = -(lineLength / 2);
        }
        else
            rseg->texScale[0] = -size;
    }
    else
    {   // Left shadow.
        rseg->texOffset[0] = xOffset;
        // Make sure the shadow isn't too big
        if(size > lineLength)
        {
