...
void Sv_WriteSectorDelta(const void* deltaPtr)
{
    ...
    if(df & SDF_FLOOR_MATERIAL)
        Msg_WritePackedShort(Materials_ToIndex(d->planes[PLN_FLOOR].surface.material));
    if(df & SDF_CEILING_MATERIAL)
        Msg_WritePackedShort(Materials_ToIndex(d->planes[PLN_CEILING].surface.material));
    if(df & SDF_LIGHT)
    {
...
void Sv_WriteSideDelta(const void* deltaPtr)
{
    ...
    if(df & SIDF_TOP_MATERIAL)
        Msg_WritePackedShort(Materials_ToIndex(d->top.material));
    if(df & SIDF_MID_MATERIAL)
        Msg_WritePackedShort(Materials_ToIndex(d->middle.material));
    if(df & SIDF_BOTTOM_MATERIAL)
        Msg_WritePackedShort(Materials_ToIndex(d->bottom.material));
