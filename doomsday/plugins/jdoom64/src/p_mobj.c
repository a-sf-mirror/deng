 ...
 */
mobj_t* GameMap_SpawnMobj3f(GameMap* map, mobjtype_t type, float x, float y,
...
mobj_t* GameMap_SpawnMobj3fv(GameMap* map, mobjtype_t type, const float pos[3],
    angle_t angle, int spawnFlags)
{
    return GameMap_SpawnMobj3f(map, type, pos[VX], pos[VY], pos[VZ], angle, spawnFlags);
}

mobj_t* P_SpawnCustomPuff(GameMap* map, mobjtype_t type, float x,
...
void P_SpawnPuff(GameMap* map, float x, float y, float z, angle_t angle)
{
    assert(map);
    {
    mobj_t* th = P_SpawnCustomPuff(map, MT_PUFF, x, y, z, angle);
...
void P_SpawnBlood(GameMap* map, float x, float y, float z, int damage,
                  angle_t angle)
{
    assert(map);
    {
    ...
    z += FIX2FLT((P_Random() - P_Random()) << 10);
    if((mo = GameMap_SpawnMobj3f(map, MT_BLOOD, x, y, z, angle, 0)))
    {
        mo->mom[MZ] = 2;
        mo->tics -= P_Random() & 3;
...
mobj_t* P_SPMAngle(mobjtype_t type, mobj_t* source, angle_t sourceAngle)
{
    ...
    if(!P_MobjIsCamera(source->player->plr->mo))
        spawnZOff = cfg.plrViewHeight - 9 + source->player->plr->lookDir / 173;
    else
        spawnZOff = 0;
