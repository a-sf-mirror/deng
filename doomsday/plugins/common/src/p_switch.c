...
void P_InitSwitchList(void)
{
    ...
    for(index = 0, i = 0; ; ++i)
    {
        ...
        switchlist[index++] = P_MaterialForName(MN_TEXTURES, switchInfo[i].name1);
        switchlist[index++] = P_MaterialForName(MN_TEXTURES, switchInfo[i].name2);
    }
...
# endif
    ...
    for(index = 0, i = 0; ; ++i)
    {
        ...
        if(SHORT(sList[i].episode) <= episode)
        {
            ...
            switchlist[index++] = P_MaterialForName(MN_TEXTURES, sList[i].name1);
            switchlist[index++] = P_MaterialForName(MN_TEXTURES, sList[i].name2);
...
void T_MaterialChanger(materialchanger_t* mchanger)
{
    if(!(--mchanger->timer))
    {
        ...
        S_StartSound(SFX_SWTCHN, DMU_GetPtrp(
            DMU_GetPtrp(mchanger->side, DMU_SECTOR), DMU_SOUND_ORIGIN));
#elif __JHERETIC__
        S_StartSound(SFX_SWITCH, DMU_GetPtrp(
            DMU_GetPtrp(mchanger->side, DMU_SECTOR), DMU_SOUND_ORIGIN));
#endif
...
int findMaterialChanger(void* p, void* data)
{
    materialchanger_t*  mchanger = (materialchanger_t*) p;
                        ...
                        int sound, boolean silent, int tics)
{
    ...
    if((mat = P_GetSwitch(current, &info)))
    {
        if(!silent)
        {
            ...
            S_StartSound(sound, DMU_GetPtrp(DMU_GetPtrp(side, DMU_SECTOR),
                                          DMU_SOUND_ORIGIN));
        }

        DMU_SetPtrp(side, ssurfaceID == SID_MIDDLE? DMU_MIDDLE_MATERIAL :
                        ssurfaceID == SID_TOP? DMU_TOP_MATERIAL :
